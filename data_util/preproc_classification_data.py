...
def readRawCommSentData(fn, verbose=True):
    with open(fn, "rt", encoding='utf-8') as csvfile:
...
def dumpClassificationData(fn="Anonymized_Sentences_Classified.csv", random_split=True):
    ...
    if random_split:
        ...
        # raw_train_comm_data
        merged_train_comm_list_temp = [" ".join(sent_list) for sent_list in train_comm_sent_list]
        print("debug: comm_label_list", comm_label_list[:5])
        merged_train_comm_list = []
        # merged_train_label_list = [np.max(label_list) for label_list in train_label_list]
        merged_train_label_list = []
        index = 0
        ...
        # raw_test_comm_data
        merged_test_comm_list_temp = [" ".join(sent_list) for sent_list in test_comm_sent_list]
        # for label_list in test_label_list:
        #     try:
        ...
        # merged_test_label_list = [np.max(label_list) or 0 for label_list in test_label_list]
        merged_test_label_list = []
        merged_test_comm_list = []
        test_comm_index = 0
        for label_list in test_label_list:
            try:
                merged_test_comm_list.append(merged_test_comm_list_temp[test_comm_index])
                merged_test_label_list.append(np.max(label_list))
            except:
                print(f"error in {merged_test_comm_list[test_comm_index]}")
                # merged_train_label_list.append(0)
            finally:
                test_comm_index += 1
        print(
            f"The original size = {len(merged_test_comm_list_temp)} and the new size = {len(merged_test_comm_list)}")
...
if __name__=="__main__":
    ...
    dumpClassificationData(param.stormfront_classification_dataset, random_split=True)
